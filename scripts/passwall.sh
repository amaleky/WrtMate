#!/bin/bash
# Passwall configuration for WrtMate

# Detect CPU architecture and FPU type
ARCH=$(uname -m)

# Check if the system is soft-float
HAS_HARD_FLOAT=1
...
# Check if CPU supports x86-64-v3 features for amd64
HAS_V3=0
if [ "$ARCH" = "x86_64" ]; then
...
passwall() {
  ...
  opkg install dnsmasq-full kmod-nft-socket kmod-nft-tproxy unzip binutils || error_exit "Failed to install Passwall dependencies."

  # Download and install Passwall packages
  ...
  if [[ "$WARP_INSTALL" != "no" ]]; then
    ...
    # Enable and start WARP services
  ...
  if [[ "$HIDDIFY_INSTALL" != "no" ]]; then
    ...
    # Create Hiddify directory if it doesn't exist
    if [ ! -d /root/hiddify/ ]; then
      mkdir /root/hiddify/
    fi

    # Install Hiddify configurations
    ...
    # Enable and start Hiddify service
    /etc/init.d/hiddify-cli enable
    /etc/init.d/hiddify-cli restart
  fi

  # Apply PassWall2 settings and restart
