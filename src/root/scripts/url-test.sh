...
if /etc/init.d/ghost enabled; then
  if [ "$(logread | grep "run.sh\[$(pgrep -f '/root/ghost/run.sh')\]" | grep -c "ERROR")" -gt 50 ] || [ "$(curl -I --max-time 1 --retry 3 --socks5-hostname "127.0.0.1:22334" --silent --output "/dev/null" -w "%{http_code}" "$TEST_SANCTION_URL")" -ne 200 ]; then
...
if /etc/init.d/warp-plus enabled; then
  if ! curl -I --max-time 1 --retry 3 --socks5-hostname "127.0.0.1:8086" --silent --output "/dev/null" "$TEST_URL"; then
...
if /etc/init.d/psiphon enabled; then
  if [ "$(curl -I --max-time 1 --retry 3 --socks5-hostname "127.0.0.1:8087" --silent --output "/dev/null" -w "%{http_code}" "$TEST_SANCTION_URL")" -ne 200 ]; then
...
if /etc/init.d/ssh-proxy enabled; then
  if ! curl -I --max-time 1 --retry 3 --socks5-hostname "127.0.0.1:1080" --silent --output "/dev/null" "$TEST_URL"; then
...
if /etc/init.d/serverless enabled; then
  if ! curl -I --max-time 1 --retry 3 --socks5-hostname "127.0.0.1:10808" --silent --output "/dev/null" "$TEST_URL"; then
